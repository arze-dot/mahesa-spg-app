...
import { ACT_GetAssetList } from '@/app/(private)/(module)/input/(features)/asset/_action/action.get.asset.list';
...
const CE_AssetAsyncSelect = ({ register, setValue, indexId }: { register: any, setValue: any, indexId: string }) => {
    ...
    const asyncOptions = useCallback(
        async (inputValue: string, callback: (options: SelectOption[]) => void) => {
            const data = await ACT_GetAssetList()
            if (!inputValue) {
            ...
            const dataMapped = dataFiltered?.map((i: any) => {
                return {
                    value: i?.name, label: i?.name, raw: i
                }
            });
    ...
    return (
        <AsyncSelect
            ...
            onChange={(selectedOption: any) => {
                setValue(indexId, selectedOption?.raw)
                setSelectedData(selectedOption);
            }}
            placeholder="Ketikan nama asset"
...
export default CE_AssetAsyncSelect;
