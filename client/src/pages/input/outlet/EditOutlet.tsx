...
import ACT_DETAIL_OUTLET from '../../../api/outlets/detail-outlet';
import ACT_EDIT_OUTLET from '../../../api/outlets/edit-outlet';
...
const EditOutlet: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (id) {
            try {
                ...
                if (file) {
                    uploadResponse = await ACT_UPLOAD(file, 'outlet');
                    if (uploadResponse.status === 201 || uploadResponse.status === 200) {
                        imageUrl = uploadResponse.data.files.outlet;
                    } else {
                ...
                const createOutletResponse = await ACT_EDIT_OUTLET(outletData);

                if (createOutletResponse.status === 200) {
                    alert('Berhasil mengedit outlet');
                    navigate('/dashboard/input/outlet');
                } else {
                    throw new Error('Gagal mengedit outlet');
                }

            } catch (error) {
                console.error(error);
                alert('Gagal mengedit outlet');
            } finally {
                setLoading(false);
            }
        }
    ...
    const getData = async () => {
        if (id)
            await ACT_DETAIL_OUTLET(id).then((res) => {
